...
LEFT OUTER JOIN `Register` AS b ON a.`stdNo` = b.`regStdNo`;
...
FROM `Student` AS a
JOIN `Register` AS b ON a.`stdNo` = b.`regStdNo`AND `regLecNo` = 349;
...
SELECT
	`stdNo`,
	`stdName`,
	GROUP_CONCAT(`lecName` ORDER BY `lecClass` DESC) AS `신청과목`,
	GROUP_CONCAT(if(`regTotalScore` >= 60, `lecName`, NULL) ORDER BY `lecClass` DESC) AS `이수과목`
FROM `Student` AS a
JOIN `Register` AS b ON a.`stdNo` = b.`regStdNo`
JOIN `Lecture` AS c ON b.`regLecNo` = c.`lecNo`
GROUP BY `stdNo`;