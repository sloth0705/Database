...
INSERT INTO `Departments` VALUES('106', '산부인과', '신가임당', '051-123-0106');
...
SELECT * 
FROM `Patients` 
WHERE (YEAR(NOW())-CONCAT(19,LEFT(`pat_jumin`, 2))) >= 30 
AND (YEAR(NOW())-CONCAT(19,LEFT(`pat_jumin`, 2))) < 40;

#문제 8
SELECT b.`doc_name`, `dep_name`
FROM `Departments` AS a
JOIN `Doctors` AS b ON a.dep_no = b.dep_no
WHERE a.dep_manager != b.doc_name;

#문제 9
SELECT GROUP_CONCAT(`pat_name` SEPARATOR '^')
FROM `nurses` AS a
JOIN `patients` AS b ON a.nur_id = b.nur_id
WHERE a.nur_name = '김태희';

#문제 10
SELECT a.`nur_id`, `nur_name`, COUNT(`pat_id`) AS `담당환자수`
FROM `nurses` AS a
JOIN `patients` AS b ON a.nur_id = b.nur_id
GROUP BY a.`nur_id`
ORDER BY `담당환자수` DESC
